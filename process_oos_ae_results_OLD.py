...
from utils_OLD import (
    autoenrollment_table_to_dict, check_autoenrollment_accuracy, 
    check_proper_autoenrollment_table, get_boolean_autoenrollment_accuracy_col,
    create_snippets_df, create_no_mention_of_autoenrollment_flag
    )

TABLE_COLS = ['auto_enrollment_offered', 'initial_deferral', 'auto_increase_offered', 'auto_increase_amount', 'auto_increase_cap']
...
def process_df(df: pd.DataFrame, model: str, result_col: str=None):
    ...
    for t in llm_results:
        ...
        good_table_bool = check_proper_autoenrollment_table(t, TABLE_COLS)

        error.append(not good_table_bool)
        if good_table_bool:
            d = autoenrollment_table_to_dict(t)
...
def main():
    ...
    agree_rate = check_autoenrollment_accuracy(df['f1_AE_table'].astype(str), df['f2_AE_table'].astype(str))

    print(f"LLMs agree on {agree_rate*100:.3f}% of plans ({agree_rate*len(df)} rows)")

    df['AE_models_agree_flag'] = get_boolean_autoenrollment_accuracy_col(df['f1_AE_table'].astype(str), df['f2_AE_table'].astype(str))

    snippets_df = create_snippets_df(['g1_AE', 'g2_AE'])
    df = df.merge(snippets_df, on='ack_id')

    df['no_mention_of_ae_flag'] = create_no_mention_of_autoenrollment_flag(df, ['g1_AE_snippet', 'g2_AE_snippet'])
    ...
    df.to_stata(SAVE_PATH / 'autoenrollment_full_dataset_unfiltered_v4.dta', write_index=False, version=118)
    df2 = df.drop(columns=['g1_AE_snippet', 'g2_AE_snippet'])
    df2.to_stata(SAVE_PATH / 'autoenrollment_full_dataset_unfiltered_no_snippets_v4.dta', write_index=False, version=118)

    df_filtered = df.loc[df['AE_models_agree_flag']].reset_index(drop=True)
    
    df_filtered.to_stata(SAVE_PATH / 'autoenrollment_full_dataset_filtered_models_agree_v4.dta', write_index=False, version=118)

    df_filtered2 = df_filtered.drop(columns=['g1_AE_snippet', 'g2_AE_snippet'])

    df_filtered2.to_stata(SAVE_PATH / 'autoenrollment_full_dataset_filtered_models_agree_no_snippets_v4.dta', write_index=False, version=118)
